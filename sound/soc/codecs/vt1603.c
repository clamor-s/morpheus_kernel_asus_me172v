...
//<ASUS-Darren1_Chang20120617+>
#ifdef CONFIG_ASUS_ENGINEER_MODE
#include "asustag_vt1603.h"
#endif
//<ASUS-Darren1_Chang20120617->
...
//<ASUS-Darren1_Chang20120718+>
#ifdef CONFIG_ASUS_ENGINEER_MODE
struct snd_soc_codec *extern_vt1603_soc_codec;
#endif
//<ASUS-Darren1_Chang20120718->
...
//<ASUS-Darren1_Chang20120718+>
#ifdef CONFIG_ASUS_ENGINEER_MODE
unsigned int vt1603_read(struct snd_soc_codec *codec, unsigned int reg)
#else
...
static int vt1603_hp_power_up(struct snd_soc_codec *codec)
{
	...
	//Mute for bug 249638	
	vt1603_mute(codec->card->rtd->codec_dai, 1);
	/* disable mono mixing */
	...
	if (!vt1603_conf.headmic_det) {
		// set HP power up
		reg = snd_soc_read(codec, VT1603_R68);
		reg &= ~(BIT0 | BIT1 | BIT4); // Sam - Unmute HP before power up
		snd_soc_write(codec, VT1603_R68, reg);
	}
	//Mute for bug 249638	
	msleep(500);
	vt1603_mute(codec->card->rtd->codec_dai, 0);
...
static void vt1603_headmic_det(struct snd_soc_codec *codec)
{
	...
	if (!(GPIO_ID_GPIO_27_20_2BYTE_VAL & BIT6)) {
...
static void vt1603_switch_to_hp(struct snd_soc_codec *codec)
{
	...
	DBG_DETAIL();
	// Sam - Avoid speaker and headphone power up at the same time
	del_timer_sync(&power_up_timer);
	cancel_work_sync(&power_up_timer_work);
...
//	if (codec->card->rtd->codec_dai->playback_active == 1){
		if (HP_PLUG_OUT == 1){
			HP_PLUG_OUT = 0;
			mod_timer(&power_up_timer, jiffies + 2000*HZ/1000);
		}
		else
...
static void vt1603_do_timer_work(struct work_struct *work)
{
	...
	if ((vt1603_conf.headmic_det) && (vt1603_classd_or_hp)) {
		if (GPIO_ID_GPIO_27_20_2BYTE_VAL & BIT6)
			cur_hook_state = 1;
		else 
			cur_hook_state = 0;
...
static void power_up_do_timer_work(struct work_struct *work)
{
	vt1603_classd_power_up(vt1603_soc_codec);
}
...
static void power_up_timer_handler(unsigned long data)
{
	schedule_work(&power_up_timer_work);
}
...
//<ASUS-Darren1_Chang20120914+>
#ifdef CONFIG_ASUS_ENGINEER_MODE
SOC_SINGLE("TEST MICL ENABLE 0", VT1603_R03, 2, 1, 0),  
SOC_SINGLE("TEST MICL ENABLE 1", VT1603_R03, 3, 1, 0),  
SOC_SINGLE("TEST MICL ENABLE 2", VT1603_R8e, 4, 1, 0),  
SOC_SINGLE("TEST MICL ENABLE 3", VT1603_R8e, 6, 1, 0),   
SOC_SINGLE("TEST MICL ENABLE 4", VT1603_R8e, 7, 1, 0),  
SOC_SINGLE("TEST MICL ENABLE 5", VT1603_R8e, 5, 1, 0),  

SOC_SINGLE("TEST MICR ENABLE 0", VT1603_R03, 2, 1, 0),  
SOC_SINGLE("TEST MICR ENABLE 1", VT1603_R03, 3, 1, 0),  
SOC_SINGLE("TEST MICR ENABLE 2", VT1603_R8e, 5, 1, 0),  
SOC_SINGLE("TEST MICR ENABLE 3", VT1603_R8e, 6, 1, 0),  
SOC_SINGLE("TEST MICR ENABLE 4", VT1603_R8e, 7, 1, 0),  
SOC_SINGLE("TEST MICR ENABLE 5", VT1603_R8e, 4, 1, 0),  

SOC_SINGLE("TEST HP POWER ENABLE 0", VT1603_R0d, 4, 1, 0),  
SOC_SINGLE("TEST HP POWER ENABLE 1", VT1603_R62, 4, 1, 0),  
SOC_SINGLE("TEST HP POWER ENABLE 2", VT1603_R68, 4, 1, 0),  

SOC_SINGLE("TEST CLD POWER ENABLE 0", VT1603_R0d, 4, 1, 0),  
SOC_SINGLE("TEST CLD POWER ENABLE 1", VT1603_R62, 4, 1, 0),  
SOC_SINGLE("TEST CLD POWER ENABLE 2", VT1603_R25, 1, 1, 0),  	

SOC_SINGLE("HP_MUTE_L", VT1603_R68, 0, 1, 1),
SOC_SINGLE("HP_MUTE_R", VT1603_R68, 1, 1, 1),

SOC_SINGLE("CLD_PD_L", VT1603_R82, 3, 1, 1),
SOC_SINGLE("CLD_PD_R", VT1603_R82, 4, 1, 1),
#endif
//<ASUS-Darren1_Chang20120914->
};
...
static int vt1603_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	...
	del_timer_sync(&power_up_timer);
        cancel_work_sync(&vt1603_timer_work);
        cancel_work_sync(&vt1603_hw_mute_work);
        cancel_work_sync(&power_up_timer_work);
...
static int vt1603_init(struct snd_soc_codec *codec)
{
	...
	INIT_WORK(&power_up_timer_work, power_up_do_timer_work);
	...
	init_timer(&power_up_timer);
	power_up_timer.data = 0;
	power_up_timer.function = power_up_timer_handler;
...
//<ASUS-Darren1_Chang20120621+>
#ifdef CONFIG_ASUS_ENGINEER_MODE
	extern_vt1603_soc_codec = codec;
	vt1603_init_status = ret;
	if(setup_proc_vt1603_entry())
...
static void vt1603_i2c_shutdown(struct i2c_client *client)
{
 	DBG_DETAIL();
