...
static struct i2c_client *mxt_client;
static struct mxt_data *globe_mxt=NULL;
u32 fw_checksum;
u8 ver_major;
u8 ver_minor;
u32 cfg_crc;
...
static void power(void); //jakie 
static bool in_suspend=0; // jakie
static bool in_worker=0; // jakie 
static int timer_cnt=3;
static struct mxt_data *mxt;

#ifdef CONFIG_MTD_WMT_SF
static int g_ModuleInstalled;
static int g_is_atmx768;
static int g_res_x, g_res_y;
#endif
#define ANTI_TOUCH 1

static struct timer_list wmt_ts_timer_debounce;
#ifdef ANTI_TOUCH
static struct timer_list wmt_ts_timer_antitouch; // jakieyeh
static int in_calibration = 0; // jakie Anit-touch
#endif

#define wmt_ts_timeout (HZ/100)*20 // jakieyeh (20s=2000)
...
static struct wmt_gpo_param_s wmt_power_gpo = {
        .name_id = 0,
        .active = 1,
        .bitmap = 0x200,/*BIT9*/
        .ctraddr = 0xFE110070,
...
static void ts_set_gpio_int(void)
{
	...
	} else if (wmt_ts_gpt.itbmp & 0xff00 || wmt_ts_gpt.itbmp == 0) {
...
static inline int ts_check_irq_status(void)
{
...
static void parse_arg(void)
{
	...
	if (retval == 0) {
		...
		++i;
		sscanf((buf+i), "%d:%d:[%x:%x:%x:%x:%x:%x:%x:%x]",
			&g_res_x,
			&g_res_y,
			&wmt_power_gpo.name_id,
			&wmt_power_gpo.active,
			&wmt_power_gpo.bitmap,
			&wmt_power_gpo.ctraddr,
			&wmt_power_gpo.ocaddr,
			&wmt_power_gpo.odaddr,
			&wmt_power_gpo.opcaddr,
			&wmt_power_gpo.opdaddr);
...
void process_T9_message(u8 *message, struct mxt_data *mxt, int last_touch)
{
	...
	if (last_touch) {
		...
		for (i = 0; i < 10; i++) {
			...
			//printk("touch %d %d %d %d\n",i,fingerInfo[i].x,fingerInfo[i].y,fingerInfo[i].pressure);
				active_touches++;
				...
				input_report_abs(mxt->input, ABS_MT_POSITION_X,
						 fingerInfo[i].x); /* jakie inverter */ 
				input_report_abs(mxt->input, ABS_MT_POSITION_Y,
						 fingerInfo[i].y); /* jakie inverter */ 
...
/*
							...
							mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_TOUCH_MULTITOUCHSCREEN_T9, mxt) + 7, 0x3C); jakie for suspend */
...
#ifdef ANTI_TOUCH
	if( message[0] == 0x1 && message[1] == 0x10)
	{
		printk("calibration message !\n"); 
		in_calibration = 0; // jakie in Anit-touch
		mod_timer(&wmt_ts_timer_antitouch, jiffies + (HZ/100)*1000); // jakieyeh 10s Anit-touch
	}
#endif
	report_id = message[0];
...
//	printk("\njakie process_message obecjt %d end \n",object );
...
static void mxt_worker(struct work_struct *work)
{
	...
	if(in_calibration == 5)
	{
		printk("Anti-touch end \n");
		in_calibration = 0;
		mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8, mxt)+6, 0xFF);  // CFG-2
		mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8, mxt)+7, 0x01); 
	...
	if(in_calibration == 6) // suspend mode
	{
		printk("Anti-touch restart \n");
		in_calibration = 0;
	...
	if(wmt_ts_timer_debounce.function)
		mod_timer(&wmt_ts_timer_debounce, jiffies + wmt_ts_timeout); // jakieyeh

	/* Make sure we don't miss any interrupts and read changeline. */
//	if(! mxt->read_chg )
//	{
//		printk("jakie worker null \n");
//	}

	//if (mxt->read_chg() == 0)
	//printk(" REG32_VAL(0xfe110000) & BIT9 = %x \n",REG32_VAL(0xfe110000));
	//while ( input == 0)
...
//	if(unlikely(d_flag))
	...
	if (1 /*mxt->valid_interrupt()*/) {
	//while( (REG32_VAL(0xfe110000) & BIT9) == 0 ) {
		/* Send the signal only if falling edge generated the irq. */
		cancel_delayed_work(&mxt->dwork);
		schedule_delayed_work(&mxt->dwork, 0);
		mxt->valid_irq_counter++;
		//printk("jakie Touch: IRQ detect %d isaddr=%x itbmp=%x \n",irq_cnt,REG32_VAL(wmt_ts_gpt.isaddr),REG32_VAL(wmt_ts_gpt.itaddr));
		//REG32_VAL(wmt_ts_gpt.itaddr) &= ~0x0200;  // set to low level trigger 
...
{
	REG32_VAL(wmt_power_gpo.ctraddr) |= wmt_power_gpo.bitmap;
	REG32_VAL(wmt_power_gpo.odaddr) &= ~wmt_power_gpo.bitmap;
	REG32_VAL(wmt_power_gpo.ocaddr) |= wmt_power_gpo.bitmap;
	mdelay(1); // >- 1us 
	REG32_VAL(wmt_power_gpo.odaddr) |= wmt_power_gpo.bitmap;
#if 0
	REG32_VAL(0xfe110070) |= 0x200;
	REG32_VAL(0xfe1100f0) &= ~0x200;
	REG32_VAL(0xfe1100b0) |= 0x200;
	mdelay(500); // >- 1us 
	REG32_VAL(0xfe1100f0) |= 0x200;
#endif
}
...
static int mxt_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	in_suspend=1; // jakie
	//REG32_VAL(0xfe1100f0) &= ~0x200;  // reset jakie 
	REG32_VAL(wmt_power_gpo.odaddr) &= ~wmt_power_gpo.bitmap;  // reset jakie 
...
static int mxt_resume(struct i2c_client *client)
{
	...
	//else
		// mxt_start(mxt);  jakie
#ifdef ANTI_TOUCH
	in_calibration = 6; // jakie Anit-touch suspend mode
	schedule_delayed_work(&mxt->dwork, 0);
#endif
...
#ifdef ANTI_TOUCH
static inline void wmt_ts_antitouch_func(unsigned long fcontext) // jakieyeh 
{
	in_calibration=5; // send end of ani-touch cmd 
	schedule_delayed_work(&mxt->dwork, 0);
}
#endif

static inline void wmt_ts_timeout_func(unsigned long fcontext) // jakieyeh 
{
	if ((REG32_VAL(0xfe110000)  & (BIT9)) != 0)  // is high 
	{	
		timer_cnt -- ;
		if( timer_cnt == 0) 
		{
			REG32_VAL(wmt_ts_gpt.itaddr) |= 0x0200; // falling edge 
			timer_cnt =3;
		}
		else
			mod_timer(&wmt_ts_timer_debounce, jiffies + wmt_ts_timeout); // jakieyeh
	...
	{ 
		REG32_VAL(wmt_ts_gpt.itaddr) &= ~0x0200; // low level 
		if(wmt_ts_timer_debounce.function)
			mod_timer(&wmt_ts_timer_debounce, jiffies + (HZ/100)*5000 /* wmt_ts_timeout*/); // jakieyeh 50s
		timer_cnt = 3;
	}

#if 0
	if(REG32_VAL(0xfe110000)  & (BIT9)) 
	{	
		printk("is hi %x , set to falling\n",REG32_VAL(0xfe110000)  & (BIT9));
		REG32_VAL(wmt_ts_gpt.itaddr) |= 0x0200;

	} 
	else
	{
		printk("is low %x , set to low\n",REG32_VAL(0xfe110000)  & (BIT9));
		REG32_VAL(wmt_ts_gpt.itaddr) &= ~0x0200;
...
// jakie	mxt->client = client;
	...
	/* Multitouch */
	input_set_abs_params(input, ABS_MT_POSITION_X, 0, 1024 /*mxt->max_x_val*/, 0, 0);
	input_set_abs_params(input, ABS_MT_POSITION_Y, 0, 600 /*mxt->max_y_val*/, 0, 0);
...
/* jakie 	__set_bit(EV_TOUCH, input->evbit); */
    ...
    /* jakie   touch_vendor_id = tegra3_query_touch_module_pcbid();	 	*/
	touch_vendor_id=0;
	...
	ts_set_gpio_int();
#ifdef ANTI_TOUCH
	in_calibration = 0; // jakie Anit-touch
#endif
	error = request_irq(wmt_ts_gpt.irq , mxt_irq_handler, IRQF_SHARED, client->name, mxt);
	error=1;
#if 0
		if (error < 0) {
...
/*init timer*/
	if(wmt_ts_gpt.itbmp == 0x200)  // falling edge 
	{
		init_timer(&wmt_ts_timer_debounce); // jakieyeh
		wmt_ts_timer_debounce.function = wmt_ts_timeout_func;
		//mod_timer(&wmt_ts_timer_debounce, jiffies + (HZ/100)*2000); // jakieyeh 20s
	}
	else
		wmt_ts_timer_debounce.function=0;

#ifdef ANTI_TOUCH
	init_timer(&wmt_ts_timer_antitouch); // jakieyeh
	wmt_ts_timer_antitouch.function = wmt_ts_antitouch_func;
	mod_timer(&wmt_ts_timer_antitouch, jiffies + (HZ/100)*2500); // jakieyeh 25s
#endif
...
static struct mxt_ts_platform_data mxt_ts_pdata = {
    .i2c_bus_id   = 1/*mxt_I2C_BUS_0*/,
    .sclk_div     = 0x04,
//    .irq_type     = 0 FALLING_EDGE_ACTIVE,
    .gpio_num     = 2,
    .panel_type   = 0/*PANEL_TYPE_4WIRED*/,
};
...
static struct i2c_board_info __initdata  mxt_ts_i2c_board_info= {
	.type          = "maXTouch",
	.flags         = 0x00,
	.addr          = 0x4b,// 4b is 640 , 768E *hi is 0x4d, low is 4c
...
static int __init mxt_init(void)
{
	...
	if (g_ModuleInstalled == 0) {
		printk("MXT touch did not install\n");
		goto release_dev;
	}
	if(parse_gpt_arg()==0) 
             goto release_dev; // jakie 
	adapter = i2c_get_adapter(mxt_ts_pdata.i2c_bus_id);
	if (NULL == adapter) {
