...
bool _API_I2C_Write(u16 writeAddress, u8 writeLength, u8 *PWriteData)
{
        ...
        if (!PWriteData) {
                dev_err(&client->dev, "%s: Write buffer pointer error.\n", __func__);
                return false;
        }
        ...
        for (i=0; i<writeLength; i++) {
                ...
                ret = ug31xx_write_i2c(client, writeAddress+i, tmp_buf, byte_flag);
                if (ret) {
                        dev_err(&client->dev, "%s: Write data(0x%02X) fail. %d\n", 
                                        __func__, i, ret);
                        return false;
                }
...
bool _API_I2C_Read(u16 readAddress, u8 readLength, u8 *pReadDataBuffer)
{
        ...
        if (!pReadDataBuffer) {
                dev_err(&client->dev, "%s: Read buffer pointer error.\n", __func__);
                return false;
        }
        ...
        for (i=0; i<readLength; i++) {
                ...
                ret = ug31xx_read_i2c(client, readAddress+i, &tmp_buf, byte_flag);
                if (ret) {
                        dev_err(&client->dev, "%s: read data(0x%02X) fail. %d\n", 
                                __func__, i, ret);
                        return false;
                }
