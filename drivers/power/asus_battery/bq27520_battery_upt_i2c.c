...
{
        int curr_cell_type, fw_cell_type;
        int ret = UPDATE_NONE;
        int curr_volt;
        ...
        BAT_DBG("(%s) current_cell_type %d, fw_cell_type %d, current_volt %d\n", 
                        __func__,
...
{
        dev_info(&i2c->dev, "[%s] register done.", __func__);
