...
int get_tv_decoder_uboot_env(void)
{
   	char varbuf2[128];
	int n;
	int varlen;
...
int tv_decoder_s_std(struct file *file, void *fh, v4l2_std_id *norm)
{
    DBG_ERR("Not support now!\n");
	return 0;
}
...
* \retval  0 if success
*/ 
static int tv_dec_open(struct inode *inode, struct file *filp)
{
  ...
  /*--------------------------------------------------------------------------
   ...
   wmt_vid_open(VID_MODE_TVDEC, &tv_decoder_env);
    ...
    /*--------------------------------------------------------------------------
        Step 3:
    --------------------------------------------------------------------------*/
  	if (request_irq(WMT_VID_IRQ ,(void *) &tv_decoder_isr, IRQF_SHARED, "wmt-tv_decoder", (void *)dev) < 0) {      
...
* \retval  0 if success
*/ 
static int tv_dec_release(struct inode *inode, struct file *filp)
{
...
{
    return tv_dec_release(NULL, filp);
}
...
* \retval  0 if success
*/ 
static int tv_dec_probe(struct platform_device *dev)
{
	int ret = 0;
	dev_t dev_no;
	struct cdev *cdev;
...
* \retval  0 if success
*/ 
static int tv_dec_remove(struct platform_device *dev)
{
	struct cdev *cdev;
...
* \retval  0 if success
*/ 
static int tv_dec_suspend(struct platform_device *dev, pm_message_t state)
{
    TRACE("Enter\n");
    switch (state.event) {
...
* \retval  0 if success
*/ 
static int tv_dec_resume(struct platform_device *dev) //struct device *dev, u32 level)
{
    DBG_ERR("Not implemented now!\n");
...
static void tv_dec_exit(void)
{
	...
	platform_driver_unregister(&tv_dec_driver);
