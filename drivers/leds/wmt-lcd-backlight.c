...
//20130125 re-add by David Wang to let brightness increase exponentially ---START--- 
    int bright_threshold=168;
    int duty_threshold=20;
    if ((b != 255) || (!g_backlight_limit.allheigh)) {
        ...
        else if (b>=20 && b<bright_threshold){
            val = ((b-20)*duty_threshold*1000)/(bright_threshold-20);
            duty = (val * g_backlight_limit.step)/1000;
            duty = (duty + g_backlight_limit.min)/1000;
        }
        else if (b>=bright_threshold){
            val = ((b-bright_threshold)*(g_backlight_limit.diff-duty_threshold)*1000)/(255-bright_threshold);
            duty = (val * g_backlight_limit.step)/1000;
            duty = (duty + g_backlight_limit.min + duty_threshold*g_backlight_limit.step)/1000;
        }
		/*val = (b*g_backlight_limit.diff*1000)/255;*/
		/*duty = (val * g_backlight_limit.step) /1000;*/
		/*duty = (duty + g_backlight_limit.min)/1000;*/
//20130125 re-add by David Wang to let brightness increase exponentially ---END---
    	if(duty) {
			if(--duty > 0xFFF)
	    		duty = 0xFFF;
    	}
