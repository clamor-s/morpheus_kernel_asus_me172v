...
int ge_stretch_blit_scl(ge_info_t *geinfo,
			const ge_surface_t *src,
			const ge_surface_t *dst,
			ge_rect_t *sr,
			ge_rect_t *dr)
{
	...
	/*
	printk("(%s) S: %08lx: %lu, %lu, %lu, %lu, %lu, %lu, %08lx\n",
                __func__,
                src->addr, src->x, src->y, src->xres, src->yres,
                src->xres_virtual, src->yres_virtual, src->pixelformat);
	printk("(%s) D: %08lx: %lu, %lu, %lu, %lu, %lu, %lu, %08lx\n",
                __func__,
                dst->addr, dst->x, dst->y, dst->xres, dst->yres,
                dst->xres_virtual, dst->yres_virtual, dst->pixelformat);
	if (sr)
		printk("sr: %d %d %d %d\n", sr->x, sr->y, sr->w, sr->h);
	if (dr)
		printk("dr: %d %d %d %d\n", dr->x, dr->y, dr->w, dr->h);
	*/
	...
	ge_helper_get_vdo_framebuf(&s, src, sr);
	ge_helper_get_vdo_framebuf(&d, dst, dr);

	vpp_set_mutex(1, 1);
	p_scl->scale_sync = 1;
	ret = vpp_set_recursive_scale(&s, &d);
	vpp_set_mutex(1, 0);
